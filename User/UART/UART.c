...
void UART1_CfgInit( uint32_t baudrate, uint8_t stopbits, uint8_t parity )
{
    ...
    /* Configure USART1 Rx (PA10) as input pull-up */
...
{
    /* uart1 transmission processing (Modified for OTA) */
    if( Uart.Tx_Flag )
    {
        // ... (This section shouldn't trigger since we bypass DMA)
    }
    else
    {
        ...
        if( Uart.Tx_RemainNum )
        {
            ...
            // ============== OTA INTERCEPT BLOCK ==============
            OTA_ProcessPacket(&UART1_Tx_Buf[ Uart.Tx_CurPackPtr ], Uart.Tx_CurPackLen);

            // Fake DMA completion to clear the buffer
    ...
    if( Uart.Rx_RemainLen )
    {
        if( Uart.USB_Up_IngFlag == 0 )
        {
            ...
            if( packlen )
            {
                ...
                Uart.USB_Up_TimeOut = 0x00;
                USBD_ENDPx_DataUp( ENDP3, &UART1_Rx_Buf[ Uart.Rx_DealPtr], packlen);
    ...
    if( Uart.USB_Up_Pack0_Flag )
    {
        if( Uart.USB_Up_IngFlag == 0 )
        {
            if( Uart.USB_Up_TimeOut >= ( DEF_UARTx_RX_TIMEOUT * 20 ) )
            {
                ...
                USBD_ENDPx_DataUp( ENDP3, &UART1_Rx_Buf[ Uart.Rx_DealPtr], 0);
